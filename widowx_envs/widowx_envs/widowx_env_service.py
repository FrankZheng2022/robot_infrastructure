...
class WidowXConfigs:
    DefaultEnvParams = {
        ...
        "camera_topics": [{"name": "/red/image_raw"},
                          ],
    }
...
class WidowXActionServer():
    ...
    def __observe(self, types: list) -> dict:
        if self.bridge_env:
            ...
            full_image = [mat_to_jpeg(obs["full_image"][i]) for i in range(len(obs["full_image"]))] # faster
            obs = {
                "image": obs["image"],
                "state": obs["state"],
...
class WidowXClient():
    ...
    def get_observation(self) -> Optional[dict]:
        ...
        res["full_image"] = [jpeg_to_mat(img) for img in res["full_image"]]
        return res
